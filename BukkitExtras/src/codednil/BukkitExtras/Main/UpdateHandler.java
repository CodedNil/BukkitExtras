...
public class UpdateHandler {
	...
	private UpdateHandler.UpdateResult result = UpdateHandler.UpdateResult.SUCCESS; // Used
																					// for
																					// determining
																					// the
																					// outcome
	 ...
	 *            updates in console
	 */
	public UpdateHandler(Plugin plugin, int id, File file, UpdateType type,
			boolean announce) {
		this.plugin = plugin;
		this.type = type;
		...
		try {
			this.url = new URL(UpdateHandler.HOST + UpdateHandler.QUERY + id);
	 ...
	 */
	public UpdateHandler.UpdateResult getResult() {
		this.waitForThread();
		return this.result;
	}
	...
	private void saveFile(File folder, String file, String u) {
		...
		try {
			...
			final byte[] data = new byte[UpdateHandler.BYTE_SIZE];
			int count;
			if (this.announce) {
			...
			long downloaded = 0;
			while ((count = in.read(data, 0, UpdateHandler.BYTE_SIZE)) != -1) {
				downloaded += count;
			...
			if (this.announce) {
				this.plugin
						.getLogger()
						.info("Finished updating. Please reload or restart the server.");
			}
			...
			this.result = UpdateHandler.UpdateResult.FAIL_DOWNLOAD;
		} finally {
			try {
				if (in != null) {
					in.close();
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (entry.isDirectory()) {
					...
					int b;
					final byte buffer[] = new byte[UpdateHandler.BYTE_SIZE];
					final FileOutputStream fos = new FileOutputStream(
							destinationFilePath);
					final BufferedOutputStream bos = new BufferedOutputStream(
							fos, UpdateHandler.BYTE_SIZE);
					while ((b = bis.read(buffer, 0, UpdateHandler.BYTE_SIZE)) != -1) {
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(" ").length == 2) {
				...
				if (this.hasTag(version)) {
					// This build is tagged for no-update
					this.result = UpdateHandler.UpdateResult.NO_UPDATE;
					return false;
				}

				final String remoteVersion = title.split(" ")[1]; // Get the
																	// newest
																	// file's
																	// version
																	// number
				...
				if (remoteValue < localValue) {
					...
					if (remoteLetterValue <= localLetterValue) {
						// Old version
						this.result = UpdateHandler.UpdateResult.NO_UPDATE;
						return false;
					}
				...
				this.plugin.getLogger().info("Found version " + remoteVersion);
				...
				this.result = UpdateHandler.UpdateResult.FAIL_NOVERSION;
				return false;
			}
		}
		return true;
	...
	private boolean hasTag(String version) {
		for (final String string : UpdateHandler.NO_UPDATE_TAG) {
	...
	private boolean read() {
		try {
			...
			conn.addRequestProperty("User-Agent", "UpdateHandler - EliteJynx");
			...
			try {
				...
				plugin.getLogger()
						.warning(
								"You are currently not connected to the internet or the file server is down");
			...
			this.versionName = (String) ((JSONObject) array
					.get(array.size() - 1)).get(UpdateHandler.TITLE_VALUE);
			this.versionLink = (String) ((JSONObject) array
					.get(array.size() - 1)).get(UpdateHandler.LINK_VALUE);
			this.versionType = (String) ((JSONObject) array
					.get(array.size() - 1)).get(UpdateHandler.TYPE_VALUE);
			this.versionGameVersion = (String) ((JSONObject) array.get(array
					.size() - 1)).get(UpdateHandler.VERSION_VALUE);
				...
				if (read()) {
					if (versionCheck(versionName)) {
						if ((versionLink != null)
							...
							saveFile(new File(plugin.getDataFolder()
									.getParent(), updateFolder), name,
									versionLink);
						} else {
